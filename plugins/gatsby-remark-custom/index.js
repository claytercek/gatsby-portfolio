...
module.exports = test => {
  ...
  visit(markdownAST, 'image', async node => {
    if (node.url.endsWith('mp4')) {
      // is vidoe
      const assetPath = path.join(__dirname, '../../static', node.url)
      ...
      try {
        const probe = await ffprobe(assetPath, {path: ffprobeStatic.path})
      ...
      // is image
      const assetPath = path.join('../../static', node.url)
      node.url = assetPath
    }
  })
